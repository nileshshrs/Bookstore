...
const EditForm = () => {
  const { fetchBooks, slideSidebar, slide, singleBook, updateBook } =
    useBookContext();
  ...
  useEffect(() => {
    if (singleBook) {
      setTitle(singleBook.title || ""); // Ensure to handle null values
      setIsbn(singleBook.isbn || "");
      setAuthor(singleBook.authorName || "");
      setPrice(singleBook.price || 0);
      setDescription(singleBook.description || "");
      setSelectedImage(singleBook.imagePath || null);
    }
  }, [singleBook]);
  ...
  const uploadImage = async (e) => {
    ...
    if (!file) {
      handleSubmit(singleBook.imagePath);
    } else {
  ...
  const handleSubmit = async (url) => {
    ...
    updateBook(singleBook.bookId, formData);
    // Fetch updated books
    fetchBooks();

    toast.success("Book has been updated sucessfully", {
      position: "top-right",
    });
    setTitle("");
    setIsbn("");
